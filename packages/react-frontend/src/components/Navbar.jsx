    ...
    try {
      ...
      if (searchBy === "Website" || searchBy === "Username") {
        if (encodeURIComponent(query)) {
          url =
            searchBy === "Website"
              ? `${API_PREFIX}/credentials/website/${encodeURIComponent(query)}`
              : `${API_PREFIX}/credentials/username/${encodeURIComponent(query)}`;
        } else {
    ...
    >
      <nav className="navbar navbar-light justify-content-between text-bold">
        ...
        <div className="d-flex justify-content-center align-items-center w-50">
          <form
            className="px-1 d-flex align-items-center w-100"
        ...
        <div className="dropdown">
          ...
          >
            <button
              ...
              type="button"
              onClick={() => handleShowModal("Option2")}
            >
              Log Out
            </button>
            <button
              className="dropdown-item"
              type="button"
              onClick={() => handleShowModal("Option3")}
            >
              Delete Account
            </button>
          </div>
        ...
        {showModal === "Option2" && (
          <Option2 show={true} onClose={handleCloseModal} />
        )}
        {showModal === "Option3" && (
          <Option3 show={true} onClose={handleCloseModal} />
        )}
      </nav>
      <div className="mt-3">
