...
describe("home page", () => {
  ...
  beforeEach("logs in to application", () => {
    ...
    cy.url().should('eq', 'https://delightful-dune-019b8a21e.5.azurestaticapps.net/') // vertification that the user has logged in
  });
  ...
  it("creates a new credential, searches for it by username, updates it, then searches for it again by website, then deletes it. Then logs out and signs up with new credentaials", () => {
    ...
    cy.wait(100)
    cy.url().should('eq', 'https://delightful-dune-019b8a21e.5.azurestaticapps.net/login')

    // navigate signup
    cy.get('.auth-link').click()
    cy.url().should('eq', 'https://delightful-dune-019b8a21e.5.azurestaticapps.net/signup')
    ...
    cy.get('.auth-button').click()
    cy.wait(400)
    cy.get('.auth-message').should('contain.text', "Signup successful for user");
